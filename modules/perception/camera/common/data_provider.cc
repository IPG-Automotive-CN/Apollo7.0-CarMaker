...
bool DataProvider::Init(const DataProvider::InitOptions &options) {
  ...
  if (options.do_undistortion) {
    ...
    ori_gray_.reset(
     ...
     AINFO<<"undistortion reset before rgb_ isNull = "<<(rgb_==nullptr);
    ori_rgb_.reset(
        new base::Image8U(src_height_, src_width_, base::Color::RGB));
    AINFO<<"undistortion reset after rgb_ isNull = "<<(rgb_==nullptr);
    ori_bgr_.reset(
...
bool DataProvider::FillImageData(int rows, int cols, const uint8_t *data,
  ...
  AINFO<<"DataProvider::false";
  gray_ready_ = false;
  rgb_ready_ = false;
  bgr_ready_ = false;

  bool success = false;
  AINFO<<"DataProvider::USEGPU";
  ...
  if (encoding == "rgb8") {
    if (handler_ != nullptr) {
      ...
      AINFO<<"11";
       AINFO<<"data.size = "<<sizeof(data);
      //AINFO<<"is NULL = "<<ori_rgb_->mutable_gpu_data()==nullptr||ori_rgb_->mutable_gpu_data()==NULL;
      AINFO<<"isNULL = "<<(rgb_==nullptr);
      AINFO<<"rows = "<<rgb_->rows();
      AINFO<<"width_step = "<<rgb_->width_step();
