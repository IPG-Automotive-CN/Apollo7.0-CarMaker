...
std::string FORMAT_TIMESTAMP_TEST(double d){
...
void ContiArsPreprocessor::SkipObjects(
  ...
  AINFO<<"raw_obstacles header timestamp = "<<FORMAT_TIMESTAMP_TEST(raw_obstacles.header().timestamp_sec());
  for (const auto& contiobs : raw_obstacles.contiobs()) {
    double object_timestamp = contiobs.header().timestamp_sec();

    AINFO<<"object timestamp = "<<FORMAT_TIMESTAMP_TEST(object_timestamp)
         <<"timestamp = "<<FORMAT_TIMESTAMP_TEST(timestamp)
         <<"timestamp+interval = "<<FORMAT_TIMESTAMP_TEST(timestamp + CONTI_ARS_INTERVAL);
