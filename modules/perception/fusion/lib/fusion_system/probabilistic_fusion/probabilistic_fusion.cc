...
void ProbabilisticFusion::FuseForegroundTrack(const SensorFramePtr& frame) {
  ...
  AINFO<<"Associate num = "<< scenes_->GetForegroundTracks().size();
  ...
  AINFO<<"UpdateAssignedTracks = "<<scenes_->GetForegroundTracks().size();
  ...
  AINFO<<"UpdateUnassignedTracks = "<<scenes_->GetForegroundTracks().size();
  ...
  AINFO<<"CreateNewTracks = "<<scenes_->GetForegroundTracks().size();
...
void ProbabilisticFusion::CreateNewTracks(
    const SensorFramePtr& frame,
    const std::vector<size_t>& unassigned_obj_inds) {
  AINFO<<"unassigned_obj_inds size  = "<<unassigned_obj_inds.size();
...
void ProbabilisticFusion::CollectFusedObjects(
  ...
  AINFO<<"foreground_tracks num = " << scenes_->GetForegroundTracks().size();
  for (size_t i = 0; i < foreground_tracks.size(); ++i) {
