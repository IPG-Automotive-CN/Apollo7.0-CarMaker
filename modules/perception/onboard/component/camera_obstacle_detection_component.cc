...
void CameraObstacleDetectionComponent::OnReceiveImage(
  ...
  if (InternalProc(message, camera_name, &error_code, prefused_message.get(),
    ...
    AINFO<<"output_final_obstacles_ = "<<output_final_obstacles_;
...
int CameraObstacleDetectionComponent::InternalProc(
  ...
  AINFO<<"yil in_message encoding = "<<in_message->encoding()<<" width = "<<in_message->width()<<"step = "<<in_message->step();
  const double msg_timestamp =
      in_message->measurement_time() + timestamp_offset_;
AINFO<<"CameraObstacleDetectionComponent 1";
  ...
  AINFO<<"CameraObstacleDetectionComponent 2";
  ...
  AINFO<<"CameraObstacleDetectionComponent 3";
  ...
  camera_frame.data_provider = data_providers_map_[camera_name].get();
   AINFO<<"FillImageData size = "<<in_message->data().size()<<" camera_name = "<<camera_name;
  ...
  AINFO<<"process success, make pb msg";
  ...
  *error_code = apollo::common::ErrorCode::OK;
  ...
  prefused_message->frame_->camera_frame_supplement.on_use = true;
   AINFO<<"FLAGS_obs_enable_visualization";
