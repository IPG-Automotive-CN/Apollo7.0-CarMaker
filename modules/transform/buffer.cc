...
void Buffer::SubscriptionCallbackImpl(
  ...
  if (now.ToNanosecond() < last_update_.ToNanosecond()) {
    ...
    for (auto& msg : static_msgs_) {
      ...
      AINFO<<"Clear and setTransform msg stamp = "<<msg.header.stamp;
  ...
  for (int i = 0; i < msg_evt->transforms_size(); i++) {
    try {
      ...
      AINFO<<"msg_evt setTransform header.seq = "<<trans_stamped.header.seq
           <<" header.frame_id = "<<trans_stamped.header.frame_id
           <<" header.stamp = "<<trans_stamped.header.stamp
           <<" child_frame_id = "<<trans_stamped.child_frame_id;
